...
interface CallingCardBuilderPrototype
{
...
final class CallingCard implements CallingCardBuilderPrototype
{
...
final class CallingCardBuilder
{
    private array $student = [];
    private string $businessName = 'College of Computing Studies';
    private string $position = 'BSIT Student';
    ...
    public function setAddress(string $street, string $city): self
    {
        $this->street = $street;
        $this->city = $city;
        return $this;
    ...
    public function build(): CallingCard
    {
        if (!isset($this->student['first_name'], $this->student['last_name'])) {
            throw new InvalidArgumentException('Each student must have first_name and last_name.');
        }

        $card = new CallingCard();
        $card->setStudent($this->student);
        $card->setBusinessName($this->businessName);
        $card->setPosition($this->position);
        $card->setAddress($this->street, $this->city);
...
echo 'Generated 1 calling card.' . PHP_EOL;
